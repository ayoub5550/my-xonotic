...
echo "== 1/6: generating synthetic fixtures =="
python3 tools/content/pk3_tool.py fixture --out-dir tests/fixtures/generated

echo "== 2/6: compiling + running BSP parser tests (Mono, no Unity) =="
BUILD_DIR="$(mktemp -d)"
...
echo "== 3/6: compiling + running static MD3 parser tests (Mono, no Unity) =="
...
echo "== 4/6: offline match rules (Mono, no Unity) =="
"$MCS" -target:exe -out:"$BUILD_DIR/MatchRulesTests.exe" \
  Assets/MyXonotic/Runtime/Gameplay/MatchRules.cs tests/csharp/MatchRulesTests.cs
"$MONO" "$BUILD_DIR/MatchRulesTests.exe"

echo "== 5/6: actual Boil entity data (not pickup runtime behavior) =="
"$MCS" -target:exe -out:"$BUILD_DIR/PickupEntityTests.exe" \
  Assets/MyXonotic/Runtime/Content/Bsp/*.cs tests/csharp/PickupEntityTests.cs
"$MONO" "$BUILD_DIR/PickupEntityTests.exe" ThirdParty/Xonotic/maps-pk3/maps/boil.bsp

echo "== 6/6: running Python PK3-safety/fixture tests =="
